    ...
    <div className="app-container flex flex-col h-full bg-[var(--color-bg-elevated)]">
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4">
        <div className="max-w-2xl mx-auto">
          {/* Client Info */}
          <div className="quotes-client-grid mb-5">
            <div>
            ...
            <div>
              <label className="block text-[10px] font-medium text-[var(--color-text-tertiary)] uppercase tracking-wider mb-1">Lugar</label>
              <input
          ...
          <div className="rounded-xl border overflow-hidden mb-4" style={{ borderColor: 'var(--color-border)' }}>
            {/* Header */}
            <div className="flex items-center px-3 py-2 text-[10px] font-medium uppercase tracking-wider text-[var(--color-text-tertiary)]" style={{ background: 'var(--color-bg-secondary)' }}>
              <span className="flex-1 min-w-0">Item</span>
              <span className="w-12 text-center shrink-0">Cant</span>
              ...
              <div key={item.id} className="flex items-center px-3 py-2 border-t" style={{ borderColor: 'var(--color-border)', background: 'var(--color-bg-elevated)' }}>
                <div className="flex-1 min-w-0 relative">
                  <select
                    ...
                    className="w-full text-xs bg-transparent text-[var(--color-text-primary)] outline-none appearance-none pr-4 cursor-pointer truncate"
                  >
                ...
                <input
                  ...
                  className="w-12 text-center text-xs bg-transparent text-[var(--color-text-primary)] outline-none shrink-0"
                />
                <span className="w-16 text-right text-xs text-[var(--color-text-secondary)] shrink-0">${item.unitPrice.toFixed(2)}</span>
                <span className="w-16 text-right text-xs font-medium text-[var(--color-text-primary)] shrink-0">${(item.quantity * item.unitPrice).toFixed(2)}</span>
                <button onClick={() => removeItem(item.id)} className="w-8 flex items-center justify-center shrink-0 text-[var(--color-text-tertiary)] hover:text-[var(--color-error)] transition-colors">
                  <Trash2 size={12} />
                </button>
              </div>
            ))}
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between items-center text-xs">
                <div className="flex items-center gap-2">
                  <span className="text-[var(--color-text-secondary)]">Envio</span>
                  <button
                  ...
                  >
                    {deliveryWaived ? 'CORTESIA' : 'Cobrar'}
                  </button>
                </div>
                <span className="text-[var(--color-text-primary)] font-medium">{deliveryWaived ? 'CORTESIA' : `$${deliveryFee.toFixed(2)}`}</span>
              </div>
              <div className="flex justify-between text-xs">
